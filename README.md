...
**下载预编译（Windows amd64）：** [Releases](https://github.com/shaoyu521/windsurf-Tools/releases) 每个版本附带：

| 文件 | 说明 |
|------|------|
| `windsurf-tools-wails.exe` | 单文件，可直接运行 |
| `windsurf-tools-wails-windows-amd64.zip` | 同上 exe 的 zip 包 |
| `SHA256SUMS.txt` | SHA256 校验（可选验证完整性） |
...
- **推荐**：从 [GitHub Releases](https://github.com/shaoyu521/windsurf-Tools/releases) 下载 `windsurf-tools-wails.exe` 或 zip（由 CI 在推送 `v*` 标签时自动构建上传，并附带 `SHA256SUMS.txt`）。
...
- **Download**: [Releases](https://github.com/shaoyu521/windsurf-Tools/releases) → `.exe`, `.zip`, `SHA256SUMS.txt`
...
仓库已配置 [GitHub Actions](.github/workflows/release-windows.yml)：当你推送 **`v` 开头** 的 tag（如 `v0.1.1`）时，会在 Windows 上自动执行 `wails build`，并创建 **GitHub Release**，上传：

- `windsurf-tools-wails.exe`
- `windsurf-tools-wails-windows-amd64.zip`
- `SHA256SUMS.txt`
